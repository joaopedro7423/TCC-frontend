...
interface IdAtiviProps {
  id: string;
}

export default function DeletAtividade(ativi: IdAtiviProps) {
  ...
  const handleDeletCampus = async () => {
    ...
    try {
      const reponse = await api.delete(`/activities/${ativi.id}`, {
        headers: {
      ...
      if (error.response) {
        toast({
          title: "Erro ao deletar a atividade!.",
